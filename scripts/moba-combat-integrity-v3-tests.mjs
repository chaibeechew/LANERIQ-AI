...
const blue=combat.match.heroes.find(h=>h.id==="blue-1"),red1=combat.match.heroes.find(h=>h.id==="red-1"),red2=combat.match.heroes.find(h=>h.id==="red-2");

// Self-cast mobility/defense must work without inventing an enemy target.
blue.x=400;blue.y=360;
const shieldBefore=blue.shield;
...
blue.x=400;blue.y=360;red1.x=500;red1.y=360;red2.x=545;red2.y=360;
const red1Before=red1.health,red2Before=red2.health;
...
assert.equal(q.ok,true);assert.deepEqual(q.hits,["red-1"]);assert.ok(combat.match.heroes.find(h=>h.id==="red-1").health<red1Before);assert.equal(combat.match.heroes.find(h=>h.id==="red-2").health,red2Before);assert.ok(q.totalDamage>0&&q.totalDamage<999999);
...
red1.x=505;red1.y=360;red2.x=535;red2.y=380;
...
assert.equal(e.ok,true);assert.ok(e.hits.includes("red-1"));assert.ok(e.hits.includes("red-2"));assert.ok(combat.match.heroes.find(h=>h.id==="red-1").statuses.slowUntil>combat.now);assert.ok(combat.match.heroes.find(h=>h.id==="red-2").statuses.slowUntil>combat.now);
...
blue.x=900;blue.y=360;blue.lastAttackAt=-Infinity;
