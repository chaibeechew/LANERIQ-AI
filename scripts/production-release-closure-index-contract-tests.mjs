...
for(const pattern of [/RELEASE_CLOSURE_BASE_SHA_INVALID/,/execFileSync\("git",\["diff","--name-only",baseSha,sha\]/,/baseMainSha:baseSha/,/fullMainPushRangeObserved:true/,/STALE_RELEASE_CANDIDATE/,/actions\/workflows\//,/event=push/,/RELEASE_CLOSURE_REQUIRED_WORKFLOW_FAILED/,/RELEASE_CLOSURE_REQUIRED_WORKFLOW_TIMEOUT/,/actions\/runs\//,/artifacts\?per_page=100/,/RELEASE_CLOSURE_ARTIFACT_DIGEST_INVALID/,/\/api\/build-info\?laneriq_closure=/,/data-dpl-id/,/doubleReadStabilityVerified:true/,/verdict:"RUNTIME_RELEASE_CLOSED"/,/globalProductionComplete:false/,/github-main-branch-protection/,/liveSchemaSecurityVerifiedByThisGate:false/,/rawMigrationTimestampEqualityRequired:false/,/normalized-SQL-fingerprint/,/production-release-closure-index\.json\.sha256/,/crypto\.createHash\("sha256"\)\.update\(bytes\)/]) assert.match(runtime,pattern);
assert.doesNotMatch(runtime,/\$\{sha\}\^1/,"Closure Index must evaluate the complete push range from github.event.before, not only HEAD^1.");
...
console.log("✓ Path-filtered App Builder/UI/Evidence/Malware gates are required only when their own push.paths match the full before→after main push range");
