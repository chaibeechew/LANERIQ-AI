...
import {buildAvatarLiveExecutionDiagnosticsV53,getAvatarLiveExecutionDiagnosticsV53Readiness} from '../lib/ai/avatar-live-execution-diagnostics-v53.js';
...
const diagnostics=buildAvatarLiveExecutionDiagnosticsV53({orchestrator,sessionTelemetry:telemetry,requiredCapabilities:['realtime3DRenderer']});assert.equal(diagnostics.readyForReleaseEvidenceInput,true);assert.equal(diagnostics.productionLive,false);
...
for(const ready of [getAvatarLiveRuntimeDriverRegistryReadiness(),getAvatarRuntimeCircuitBreakerReadiness(),getAvatarLiveProbeRunnerReadiness(),getAvatarSessionTelemetryReadiness(),getAvatarLiveRuntimeOrchestratorV53Readiness(),getAvatarLiveExecutionDiagnosticsV53Readiness()])assert.equal(ready.codeReady,true);
console.log('LANERIQ Avatar v5.3 live execution gate passed: injectable real runtime drivers, signed source-specific probe verification, timeout/circuit isolation, real session latency/lip-sync/frame telemetry, release-bound orchestration, diagnostics and deterministic local fallback are CODE-ready while real external drivers and Production LIVE remain evidence-gated.');
