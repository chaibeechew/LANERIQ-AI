...
for(const surface of LANERIQ_SYSTEM_SURFACES){const run=createSystemSurfaceCognitiveRun(surface,{goal:`Operate ${surface} safely`});assert.equal(run.constitution.lawDigest,law.lawDigest);}
...
assert.throws(()=>issueConstitutionalExecutionToken({scope:"prod",action:"deploy",principal:"user-1",risk:"critical",constitutionalAlignmentAccepted:true,humanApproved:false},secret),/HUMAN_APPROVAL_REQUIRED/);
