...
for(const pattern of [/supabase\/server\.js/,/supabase\/admin\.js/,/auth\.getUser\(\)/,/\.eq\('owner_id',userId\)/])assert.match(context,pattern);
for(const pattern of [/Trusted same-origin request required/,/getCurrentCustomerPaymentUser/,/getOwnedCustomerPaymentApp/,/email_confirmed_at/,/sensitiveKycStoredByLaneriq:\s*false/])assert.match(accountRoute,pattern);
assert.doesNotMatch(accountRoute,/lib\/supabase\/|@supabase\//);
for(const pattern of [/Trusted same-origin request required/,/getCurrentCustomerPaymentUser/,/Stripe Connect environment mismatch/,/clientSecret/,/publishableKey/])assert.match(sessionRoute,pattern);
assert.doesNotMatch(sessionRoute,/lib\/supabase\/|@supabase\//);
