...
for(const pattern of [/auth\.getUser\(\)/,/email_confirmed_at/,/Trusted same-origin request required/,/\.eq\('owner_id',\s*user\.id\)/,/sensitiveKycStoredByLaneriq:false/])assert.match(accountRoute,pattern);
