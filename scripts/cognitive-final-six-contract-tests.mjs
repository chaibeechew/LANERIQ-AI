...
const coverageBlock=evaluateProductionCognitiveGate({...baseGate,allProductCognitiveCoverageVerified:false});assert.equal(coverageBlock.mayClaimProductionCognitiveClosed,false);assert.ok(coverageBlock.failed.includes("allProductCognitiveCoverageVerified"));
const tokenBlock=evaluateProductionCognitiveGate({...baseGate,constitutionalExecutionTokenVerified:false});assert.equal(tokenBlock.mayClaimProductionCognitiveClosed,false);assert.ok(tokenBlock.failed.includes("constitutionalExecutionTokenVerified"));
const assuranceBlock=evaluateProductionCognitiveGate({...baseGate,assuranceControlPlaneVerified:false});assert.equal(assuranceBlock.mayClaimProductionCognitiveClosed,false);assert.ok(assuranceBlock.failed.includes("assuranceControlPlaneVerified"));
const supplyBlock=evaluateProductionCognitiveGate({...baseGate,supplyChainAttestationVerified:false});assert.equal(supplyBlock.mayClaimProductionCognitiveClosed,false);assert.ok(supplyBlock.failed.includes("supplyChainAttestationVerified"));
const evalBlock=evaluateProductionCognitiveGate({...baseGate,continuousEvaluationVerified:false});assert.equal(evalBlock.mayClaimProductionCognitiveClosed,false);assert.ok(evalBlock.failed.includes("continuousEvaluationVerified"));
const protocolBlock=evaluateProductionCognitiveGate({...baseGate,mcpEnabled:true,mcpExternalConformanceVerified:false});assert.equal(protocolBlock.mayClaimProductionCognitiveClosed,false);assert.ok(protocolBlock.failed.includes("mcpExternalConformanceVerifiedIfEnabled"));
