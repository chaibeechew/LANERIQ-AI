...
const legacyProviderCallIndex=generate.indexOf('generateExternalImages(');
const hardenedProviderCallIndex=generate.indexOf('runMarketHardenedImageGeneration(');
const providerCallIndex=[legacyProviderCallIndex,hardenedProviderCallIndex].filter(index=>index>=0).sort((a,b)=>a-b)[0]??-1;
...
assert.ok(legacyProviderCallIndex>=0&&hardenedProviderCallIndex>=0,'Image Studio must retain legacy provider compatibility and expose the hardened provider path.');
...
console.log('Image Studio contract passed: auth, replay-safe legacy/hardened provider execution, durable private provider capture, bounded output hosts, automatic credit refund, mobile request recovery and private Asset Library persistence are locked.');
