...
assert.match(cloudDomain,/loadBuilderGameCapacityContext/);assert.match(cloudDomain,/getAdapter\(\)\.loadGameCapacityContext/);assert.match(cloudDomain,/gameCapacityContextMigrated:\s*true/);
...
const missingContinuity=validateMobaMigrationHandoff({plan:migration,sourceFinal:{matchId:"m-1"},targetRestored:{matchId:"m-1",authoritative:true},sourceStopped:true});assert.equal(missingContinuity.ok,false,"Missing snapshot/sequence continuity must fail closed.");
