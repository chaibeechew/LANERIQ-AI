...
export const LANERIQ_COGNITIVE_SERVICE_VERSION="1.1.0";
...
export function evaluateDomainCognitiveJudge(domain,input={}){
  profileFor(domain);
  return evaluateFeatureJudge(domain,input);
}

export async function executeDomainCognitiveSelfHeal(domain,input={},deps={}){
  profileFor(domain);
  return executeCognitiveSelfHeal(domain,input,deps);
}

export function finalizeDomainCognitiveResult(domain,input={}){
  const {key}=profileFor(domain);
  const featureJudge=input.featureChecks?evaluateFeatureJudge(key,input.featureChecks):null;
  const closed=closeCognitiveRun({...input,category:input.category||key,critical:input.critical===true||key==="production-release",risk:input.risk||COGNITIVE_DOMAIN_PROFILES[key].risk});
  return Object.freeze({...closed,featureJudge,accepted:closed.verdict.accepted&&(featureJudge?featureJudge.accepted:true)});
}

export function getCognitiveDomainStatus(){
  return Object.freeze({version:LANERIQ_COGNITIVE_SERVICE_VERSION,domains:Object.keys(COGNITIVE_DOMAIN_PROFILES),providerIndependent:true,productionAutonomyUnbounded:false,productionReleaseRequiresHumanApproval:true,failureMemoryDatabasePersistence:"migration-and-adapter-ready-not-production-applied",durableCognitiveLedger:"adapter-ready-not-production-verified",realMultiProviderBenchmark:"runtime-harness-ready-production-attestation-required",featureSpecificJudges:true,boundedCognitiveSelfHeal:true,productionCognitiveGate:true});
}
