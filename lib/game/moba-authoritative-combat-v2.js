...
export function submitMobaCombatIntent(state,input={}){
  ...
  if(kind==="basic_attack"){
    if(state.now-(source.lastAttackAt||-Infinity)<state.config.hero.attackCooldown){violation(state,checked.pid,"attack_cooldown_bypass",2);return remember(state,checked,{ok:false,reason:"attack_cooldown"});}
