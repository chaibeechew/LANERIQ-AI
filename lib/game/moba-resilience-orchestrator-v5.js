...
function exactInteger(value){const n=Number(value);return Number.isFinite(n)?Math.floor(n):null;}
...
export function validateMobaMigrationHandoff({plan,sourceFinal={},targetRestored={},sourceStopped=false}={}){
  ...
  const sourceSnapshot=exactInteger(sourceFinal?.snapshotVersion),targetSnapshot=exactInteger(targetRestored?.snapshotVersion),sourceSequence=exactInteger(sourceFinal?.serverSequence),targetSequence=exactInteger(targetRestored?.serverSequence);
  const checks={snapshotExact:sourceSnapshot!==null&&targetSnapshot!==null&&targetSnapshot===sourceSnapshot&&sourceSnapshot>=integer(plan.snapshotVersion),sequenceExact:sourceSequence!==null&&targetSequence!==null&&targetSequence===sourceSequence&&sourceSequence>=integer(plan.lastServerSequence),matchIdentity:text(targetRestored?.matchId,160)===text(plan.matchId,160),sourceStopped:sourceStopped===true,targetAuthoritative:targetRestored?.authoritative===true};
