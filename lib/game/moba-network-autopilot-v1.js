...
export function simulateMobaSoakReport({capacityReport,durationMinutes=30,tickRate=20,injectedCrashEvents=0,invariantViolations=0,uncaughtExceptions=0,reconnectFailures=0,tickOverruns=0}={}){const report=capacityReport&&typeof capacityReport==="object"?capacityReport:{},duration=Math.max(1,finite(durationMinutes,30)),matches=Math.max(1,Math.floor(finite(report.targetConcurrentMatches,1))),tick=positiveInt(tickRate,20),counters={injectedCrashEvents:Math.max(0,Math.floor(finite(injectedCrashEvents))),invariantViolations:Math.max(0,Math.floor(finite(invariantViolations))),uncaughtExceptions:Math.max(0,Math.floor(finite(uncaughtExceptions))),reconnectFailures:Math.max(0,Math.floor(finite(reconnectFailures))),tickOverruns:Math.max(0,Math.floor(finite(tickOverruns)))};const faults=Object.values(counters).reduce((sum,value)=>sum+value,0),withinModeledHeadroom=report.capacityKnown===true&&finite(report.targetConcurrentPlayers)>0&&finite(report.targetConcurrentPlayers)<=finite(report.modeledStablePlayers),syntheticSoakPassed=withinModeledHeadroom&&faults===0;return Object.freeze({version:MOBA_NETWORK_AUTOPILOT_V1.version,evidenceLevel:"synthetic_soak",durationMinutes:duration,modeledMatchTicks:Math.round(matches*tick*duration*60),withinModeledHeadroom,counters:Object.freeze(counters),syntheticSoakPassed,stabilityGrade:syntheticSoakPassed?"model_pass":"model_attention_required",crashFreeClaimAllowed:false,productionSmoothnessClaimAllowed:false,truthRule:"A synthetic soak report can catch deterministic/invariant failures and quantify modeled work, but zero modeled faults is not proof of zero crashes or bugs in LIVE multiplayer."});}
