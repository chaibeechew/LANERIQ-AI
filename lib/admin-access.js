...
export function controlTowerRoleFromAppMetadata(appMetadata) {
  if (!appMetadata || typeof appMetadata !== "object") return "";
  return normalizeInternalRole(appMetadata.control_tower_role || appMetadata.role);
}

export function controlTowerRoleFromUser(user) {
  return controlTowerRoleFromAppMetadata(user?.app_metadata);
}
