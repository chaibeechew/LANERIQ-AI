...
function thermalSafe(value){const state=clean(value,24).toLowerCase();return ["nominal","cool","safe","fair","moderate"].includes(state);}
...
export function decideLaneriqExecutionPath({
  ...
  return{
    ...
    unknownThermalAllowsOwnDevice:false,
...
- Unknown mobile thermal state is not evidence of safe own-device execution; degrade/offload until safe telemetry exists.
