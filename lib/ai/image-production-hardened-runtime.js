...
async function captureHttpsImage(raw){
  ...
  const timeout=withTimeout(20000);let response;
  try{response=await fetch(String(raw),{cache:'no-store',redirect:'error',signal:timeout.signal,headers:{Accept:'image/png,image/jpeg,image/webp'}});}catch(error){if(error?.name==='AbortError')throw new ImageGenerationGatewayError('Hardened provider image capture timed out.','IMAGE_HARDENED_CAPTURE_TIMEOUT',504);throw new ImageGenerationGatewayError('Hardened provider image capture failed.','IMAGE_HARDENED_CAPTURE_FAILED',502);}finally{timeout.done();}
...
async function observeImageCandidate({task,input,requestId,providerId,output,config}){
  ...
  const timeout=withTimeout(config.observerTimeoutMs);let response;
  try{response=await fetch(endpoint,{method:'POST',headers,cache:'no-store',redirect:'error',signal:timeout.signal,body:JSON.stringify({schemaVersion:1,requestId,task,providerId,prompt:clean(input?.prompt,4000),style:clean(input?.style,100)||null,palette:clean(input?.palette,100)||null,images})});}catch(error){if(error?.name==='AbortError')throw new ImageGenerationGatewayError('Image quality observer timed out.','IMAGE_OBSERVER_TIMEOUT',504);throw new ImageGenerationGatewayError('Image quality observer is unavailable.','IMAGE_OBSERVER_UNREACHABLE',503);}finally{timeout.done();}
