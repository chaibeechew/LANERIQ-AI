...
function actionFor(signal={}){
  ...
  if(type==="provider_failure")return{action:"verified_free_failover",reason:"provider-unavailable",autoExecutable:true};
...
export function buildSelfHealingPlanV2({signals=[],mode="balanced",exactSha="",capabilityStage="code_ready"}={}){
  const normalized=list(signals).slice(0,32).map(signal=>({type:clean(signal?.type,48),severity:clean(signal?.severity,16)||"warning",evidence:clean(signal?.evidence,120)}));
  ...
  for(const item of actions){
    if(zeroMode&&item.action==="verified_free_failover")item.reason="verified-free-only-failover";
