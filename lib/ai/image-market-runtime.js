...
const MIN_VERIFIED_OUTPUTS=20;
const MIN_VERIFIED_QUALITY_SCORE=88;
...
export function getImageMarketRuntimeReadiness(env=process.env){
  ...
  const verifiedQualityScore=boundedNumber(env.IMAGE_GENERATION_VERIFIED_QUALITY_SCORE,0,100,0);
  ...
  const refundFailureRate=boundedNumber(env.IMAGE_MARKET_REFUND_FAILURE_RATE,0,1,1);
  const idempotencyVerified=bool(env.IMAGE_MARKET_IDEMPOTENCY_VERIFIED);
  const rateLimitVerified=bool(env.IMAGE_MARKET_RATE_LIMIT_VERIFIED);
  const alternateProviderAvailable=bool(env.IMAGE_MARKET_ALTERNATE_PROVIDER_AVAILABLE);
  const providerFailoverVerified=!alternateProviderAvailable||bool(env.IMAGE_MARKET_PROVIDER_FAILOVER_VERIFIED);
  ...
  const browserVerified=bool(env.IMAGE_MARKET_BROWSER_VERIFIED);
  const mobileVerified=bool(env.IMAGE_MARKET_MOBILE_VERIFIED);
  const abuseSuitePassed=bool(env.IMAGE_MARKET_ABUSE_SUITE_PASSED);
  const monitoringReady=bool(env.IMAGE_MARKET_MONITORING_READY);
  const mainSha=clean(env.IMAGE_MARKET_MAIN_SHA,64).toLowerCase();
  const productionSha=clean(env.IMAGE_MARKET_PRODUCTION_SHA,64).toLowerCase();
  const exactShaVerified=/^[a-f0-9]{40,64}$/.test(mainSha)&&mainSha===productionSha;
  const marketApproved=bool(env.IMAGE_MARKET_RELEASE_APPROVED);

  const liveProviderVerified=verifiedOutputCount>=MIN_VERIFIED_OUTPUTS&&verifiedQualityScore>=MIN_VERIFIED_QUALITY_SCORE&&Boolean(productionEvidenceId);
  const reliabilityVerified=sampleSize>=100&&successRate>=0.98&&p95Ms>0&&p95Ms<=IMAGE_GENERATION_LIMITS.timeoutMs&&refundFailureRate===0&&idempotencyVerified&&rateLimitVerified&&providerFailoverVerified;
  const releaseVerified=Boolean(e2eEvidenceId&&releaseEvidenceHash)&&refundVerified&&browserVerified&&mobileVerified&&abuseSuitePassed&&monitoringReady&&exactShaVerified&&marketApproved;
  const marketReady=observer.mode==='enforce'&&executionReady&&liveProviderVerified&&reliabilityVerified&&releaseVerified;
  ...
  if(!browserVerified)blockers.push('browser-verification-missing');
  if(!mobileVerified)blockers.push('mobile-verification-missing');
  if(!abuseSuitePassed)blockers.push('abuse-safety-suite-missing');
  if(!monitoringReady)blockers.push('monitoring-readiness-missing');
  if(!exactShaVerified)blockers.push('production-exact-sha-mismatch-or-missing');
  ...
  return Object.freeze({
    ...
    observerConfigured:observer.configured,observerKind:observer.observerKind,verifiedOutputCount,verifiedQualityScore,productionEvidenceId,
    minimumVerifiedOutputs:MIN_VERIFIED_OUTPUTS,minimumVerifiedQualityScore:MIN_VERIFIED_QUALITY_SCORE,
    reliability:Object.freeze({sampleSize,successRate,p95Ms,refundFailureRate,idempotencyVerified,rateLimitVerified,alternateProviderAvailable,providerFailoverVerified,verified:reliabilityVerified}),
    refundVerified,e2eEvidenceId,releaseEvidenceHash,
    release:Object.freeze({browserVerified,mobileVerified,abuseSuitePassed,monitoringReady,mainSha:mainSha||null,productionSha:productionSha||null,exactShaVerified,approved:marketApproved,verified:releaseVerified}),
    blockers:Object.freeze(blockers),
    rule:'Market Ready is fail-closed: enforce mode, >=20 real provider outputs at >=88 verified quality, trusted signed observation, >=100 reliability samples at >=98% success, zero refund failures, idempotency/rate-limit/failover evidence where applicable, browser/mobile/abuse/monitoring proof, Production E2E evidence and exact main=Production SHA are all required.'
  });
}
...
async function decodeArtifact(item){
  ...
  return{mime,buffer,artifactHash,canonicalImage:`data:${mime};base64,${buffer.toString('base64')}`,width:Number(item?.width)||null,height:Number(item?.height)||null};
}
...
async function observeImage({task,input,requestId,item,observer}){
  ...
  item.image=artifact.canonicalImage;
  item.expectedSha256=artifact.artifactHash;
  let endpoint;
