...
export async function openSuperGameDataAdapterSession(){
  ...
  return {
    ...
    livingWorld:{
      ...
      async applyEventAtomic({projectId,expectedRevision,eventKey,eventType,payload,nextState}){ensure();const{data,error}=await supabase.rpc("server_apply_living_world_event",{p_project_id:projectId,p_expected_revision:expectedRevision,p_event_key:eventKey,p_event_type:eventType,p_payload:payload||{},p_next_state:nextState||{}});failure(error);return data;},
