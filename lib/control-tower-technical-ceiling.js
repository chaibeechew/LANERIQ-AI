...
function freshness(item, maxAgeMinutes, now) {
  ...
  const ageMinutes = Math.max(0, Math.round((nowMs - capturedMs) / MINUTE));
  return {
...
export function evaluateControlTowerDisasterRecovery({
  ...
  const rtoMet = restoreMinutes !== null && restoreMinutes <= targetRtoMinutes;
  const rpoMet = rpoMinutes !== null && rpoMinutes <= targetRpoMinutes;
  const chaosRtoMet = recoveryMinutes !== null && recoveryMinutes <= targetRtoMinutes;
...
export function evaluateControlTowerSupplyChain({ items = [], now = new Date().toISOString() } = {}) {
  ...
  const noCritical = critical === 0;
  const highWithinPolicy = high !== null && high <= 0;
  const checks = [fresh.fresh, sbomVerified, provenanceVerified, lockVerified, noCritical, highWithinPolicy];
...
export function computeControlTowerTechnicalCeiling({
  ...
  const overall = Math.round(
    operational.overall * 0.3 +
    disasterRecovery.score * 0.2 +
