-- Defense-in-depth authority guards for direct authenticated database access.
...
create or replace function public.control_tower_guard_evidence_immutability()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if old.item_type = 'evidence' then
    raise exception 'Control Tower evidence is immutable; register a new snapshot instead'
...
revoke all on function public.control_tower_guard_evidence_immutability() from public, anon, authenticated;

create trigger control_tower_gate_authority_guard
before insert or update on public.control_tower_release_gates
for each row execute function public.control_tower_guard_gate_authority();

create trigger control_tower_evidence_immutability_guard
before update or delete on public.control_tower_items
for each row execute function public.control_tower_guard_evidence_immutability();
