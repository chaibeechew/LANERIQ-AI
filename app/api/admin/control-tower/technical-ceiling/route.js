  ...
  try {
    ...
    try {
      ...
      if (isControlTowerStorageMissing(error)) {
        return controlTowerJson({ storageReady: false, technicalCeiling: null, capabilities: auth.capabilities }, 503);
      }
      throw error;
    }
    ...
    const technicalCeiling = computeControlTowerTechnicalCeiling({ ...bundle, liveStatus });
    return controlTowerJson({ storageReady: true, release: bundle.release, liveStatus, technicalCeiling, capabilities: auth.capabilities });
  } catch {
    return controlTowerJson({ error: "Unable to evaluate technical-ceiling readiness." }, 500);
  }
}
