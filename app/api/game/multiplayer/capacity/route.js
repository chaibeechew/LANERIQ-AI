...
import {buildMobaCreatorProviderConnectStatus} from "../../../../../lib/game/moba-provider-connect-readiness-v14.js";
import {buildMobaV15V18CreatorStatus} from "../../../../../lib/game/moba-production-certification-v18.js";
...
export async function POST(request){
  try{
    ...
    const connection=buildMobaCreatorProviderConnectStatus({providerConfig:provider});
    const completion=buildMobaV15V18CreatorStatus({buildSha:String(process.env.VERCEL_GIT_COMMIT_SHA||"").trim()});
    return json({success:true,version:MOBA_RESILIENCE_ORCHESTRATOR_V5.version,qualificationVersion:qualification.version,connectionVersion:connection.version,completionVersion:completion.version,scope:appId?"owned_game":"prebuild_plan",appId:context?.project?.id||null,mode,region,targetConcurrentPlayers,targetConcurrentMatches,architecture:{playersPerMatch:10,providerRouterManaged:true,authoritativeServerRequired:true,relayRequired:true,matchmakingRequired:true,autoscalingPlanned:true,multiRegionCapable:true,dedicatedLanerIqServerRequired:false,creatorServerConfigurationRequired:false},provider:{connected:provider.configured===true,blockedByCostPolicy:provider.blockedByCostPolicy===true,costClass:provider.costClass,endpointExposed:false,credentialExposed:false},autoscale,loadRamp,simulation,certification,creatorReport,qualification,connection,completion,liveExecutionAvailable:provider.configured===true&&!provider.blockedByCostPolicy,productionEvidenceVerified:false,truthRule:"Normal creators do not need to configure server endpoints or credentials. V15 automates real Provider smoke qualification, V16 evaluates live telemetry/self-healing, V17 defines the real-device network matrix and V18 blocks release until exact-SHA external evidence is complete. Simulation or CI alone never becomes Live Production proof."});
