...
export async function POST(request){
  try{
    ...
    return json({success:true,version:MOBA_RESILIENCE_ORCHESTRATOR_V5.version,qualificationVersion:qualification.version,connectionVersion:connection.version,scope:appId?"owned_game":"prebuild_plan",appId:context?.project?.id||null,mode,region,targetConcurrentPlayers,targetConcurrentMatches,architecture:{playersPerMatch:10,providerRouterManaged:true,authoritativeServerRequired:true,relayRequired:true,matchmakingRequired:true,autoscalingPlanned:true,multiRegionCapable:true,dedicatedLanerIqServerRequired:false,creatorServerConfigurationRequired:false},provider:{connected:provider.configured===true,blockedByCostPolicy:provider.blockedByCostPolicy===true,costClass:provider.costClass,endpointExposed:false,credentialExposed:false},autoscale,loadRamp,simulation,certification,creatorReport,qualification,connection,liveExecutionAvailable:provider.configured===true&&!provider.blockedByCostPolicy,productionEvidenceVerified:false,truthRule:"Normal creators do not need to configure server endpoints or credentials. Pre-build planning is topology-only; V13 automates five Production qualification layers and V14 exposes only creator-safe connection blockers/next actions. LIVE Production still requires real provider, Preview, measured load/failover/device and exact-SHA evidence."});
