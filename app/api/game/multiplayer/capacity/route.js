...
export async function POST(request){
  try{
    ...
    const appId=String(body?.appId||"").trim();let game=null;if(appId){if(!UUID.test(appId))return json({error:"A valid Game project is required when appId is supplied."},400);game=await ownedGame(supabase,user.id,appId);if(!game)return json({error:"Owned mobile Game project not found."},404);}
