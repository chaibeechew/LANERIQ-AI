...
export async function POST(request){
  try{
    ...
    if(item.buyout){if(!appId)return reply({error:'Buyout requires an eligible owned project.'},400);const{data:app,error:appError}=await client.from('apps').select('id,owner_id,project_type,publish_status').eq('id',appId).eq('owner_id',user.id).maybeSingle();if(appError||!app)return reply({error:'Project not found.'},404);if(String(app.project_type||'').toLowerCase()==='game'||String(app.publish_status||'').toLowerCase()==='published')return reply({error:'This project is not eligible for Buyout checkout.'},409);}
