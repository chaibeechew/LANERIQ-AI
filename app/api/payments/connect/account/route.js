...
} from '../../../../../config/customer-payment-connect-policy.js';
import {
...
async function syncExisting(userId, row) {
  ...
  await saveCustomerConnectAccount({ userId, status });
  return status;
}
...
export async function GET() {
  try {
    ...
    if (!runtime.configured) {
      return json({
        ok: true,
        state: row.onboarding_state || 'incomplete',
        configured: false,
        live: row.livemode === true,
        readyForPayments: false,
        accountId: row.stripe_account_id,
      });
    }

    const status = await syncExisting(user.id, row);
    return json({ ok: true, ...status, configured: true });
  } catch (error) {
    return json(
      { ok: false, error: String(error?.message || 'Connect status unavailable.').slice(0, 220) },
...
export async function POST(request) {
  try {
    ...
    const existing = await getCustomerConnectAccount(user.id);
    if (existing) {
      const status = await syncExisting(user.id, existing);
      return json({ ok: true, reused: true, ...status });
    }
    ...
    const displayName = input.displayName || String(
      app?.name || user.email.split('@')[0] || 'LANERIQ Creator',
    ).slice(0, 120);

    const remote = await createStripeConnectedMerchant({
      userId: user.id,
      email: user.email,
      displayName,
    ...
    const status = normalizeStripeMerchantStatus(remote);
    await saveCustomerConnectAccount({ userId: user.id, status });
    ...
    return json({
      ...
      autoPrefilled: {
        contactEmail: true,
        displayName: true,
        country: Boolean(input.country),
      },
      sensitiveKycStoredByLaneriq: false,
    });
  } catch (error) {
    return json(
      { ok: false, error: String(error?.message || 'Connect setup failed.').slice(0, 220) },
      Number(error?.status) || 503,
    );
  }
}
